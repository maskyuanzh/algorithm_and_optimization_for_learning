...
对于零均值的输入 $z \sim \mathcal{N}(0, \sigma_z^2)$，恰好一半的值被置零。我们需要计算 ReLU 输出的\textbf{二阶矩}（而非方差，原因见下文）：
\begin{equation}
    \mathbb{E}[\text{ReLU}(z)^2] = \Pr(z > 0) \cdot \mathbb{E}[z^2 \mid z > 0] = \frac{1}{2} \cdot \sigma_z^2 
\end{equation}
...
因此：
\begin{equation}
    \boxed{\mathbb{E}[\text{ReLU}(z)^2] = \frac{1}{2}\,\mathbb{E}[z^2] = \frac{1}{2}\text{Var}[z]}
\end{equation}

ReLU 将\textbf{二阶矩}减半。

ReLU 的输出有非零均值 $\mathbb{E}[\text{ReLU}(z)] = \sigma_z / \sqrt{2\pi}$，因此 $\text{Var}[\text{ReLU}(z)] = \frac{1}{2}\sigma_z^2 - \frac{\sigma_z^2}{2\pi} \approx 0.34\,\sigma_z^2$，并\textbf{不是}精确的一半。但在方差传播公式中，由于下一层权重 $W$ 零均值，$\text{Var}[Wh] = \sigma_W^2 \cdot \mathbb{E}[h^2]$，出现的是 $h$ 的\textbf{二阶矩}而非方差。而 $\mathbb{E}[\text{ReLU}(z)^2] = \frac{1}{2}\sigma_z^2$ 是\textbf{精确}的。这就是 He 初始化中 $2$ 的真正来源。

\subsubsection{He/Kaiming 初始化}

加入 ReLU 后，前向二阶矩传播变为：
\begin{equation}
    \mathbb{E}[(h^{(l+1)})^2] = n_{\text{in}} \cdot \sigma_W^2 \cdot \frac{1}{2} \cdot \mathbb{E}[(h^{(l)})^2]
\end{equation}
...
对比：Xavier 给出 $\sigma_W^2 \approx 1/n_{\text{in}}$（忽略 $n_{\text{out}}$ 修正），He 给出 $\sigma_W^2 = 2/n_{\text{in}}$——正好是 Xavier 的两倍，补偿了 ReLU 的二阶矩减半。

\subsection{三种初始化的总结}

\begin{center}
\begin{tabular}{l|ccc}
激活函数 & 二阶矩缩放因子 & 前向保持条件 & 初始化 \\
\hline
...
$L$ 层等宽网络（每层宽度 $n$），ReLU 激活。每层的二阶矩放大因子为 $\frac{n\sigma_W^2}{2}$。$L$ 层后：
\begin{equation}
    \mathbb{E}[(h^{(L)})^2] = \left(\frac{n \sigma_W^2}{2}\right)^L \cdot \mathbb{E}[x^2]
\end{equation}

\begin{center}
\begin{tabular}{c|ccc}
初始化 & $n\sigma_W^2/2$ & $L = 50$ 层后二阶矩 & 状态 \\
\hline
...
输出二阶矩 $= 0.025$（衰减 40 倍！\textbf{信号消失}）。

\textbf{(B) He 初始化}：

\begin{center}
\begin{tabular}{c|cccc}
层 & $n_{\text{in}}$ & $\sigma_W^2 = 2/n_{\text{in}}$ & $n_{\text{in}}\sigma_W^2$ & 累积二阶矩 \\
\hline
输入 & — & — & — & 1 \\
...
输出二阶矩 $\approx 1$（\textbf{稳定}！最后一层没 ReLU 所以是 2）。

\subsection{初始化与 Hessian 曲率的关系}

两层线性网络 $f = vux$，Hessian $H = \begin{pmatrix} v^2 & 2uv-1 \\ 2uv-1 & u^2 \end{pmatrix}$。
...
$f(x;u,v) = vux$，$L(u,v) = \frac{1}{2}(vu-1)^2$。
...
此时 $\lambda_{\max}\approx 25.5$，$\eta_{\max}=0.078$——仍略超目标学习率 $\eta=0.1$，但参数已进入快速收敛区域：第4步 $\eta=0.1$ 一步跳到 $(0.65,0.65)$，Loss $\to 0.17$，$\lambda_{\max}$ 骤降至 $0.6$，此后完全稳定。
...
2 & 1.17 & 2.34 & 0.31 & Warmup ($\eta=0.5$) \\
3 & 0.51 & 1.14 & 0.042 & 切到 $\eta=2$ \\
4 & $\approx 0.66$ & $\approx 1.50$ & $\approx 0.0001$ & $\eta=2$ \\
\end{tabular}
\end{center}

Warmup 2 步后参数进入小曲率区域，切到大学习率后一步几乎收敛到最优解。
...
\hline
$s^{2(L-1)}$ & 2.25 & 5.06 & 25.6 & 1478 \\
$L \cdot s^{2(L-1)}$ & 4.5 & 15.2 & 128 & 14780 \\
$\eta_{\max} = 2/(L\cdot s^{2(L-1)})$ & 0.44 & 0.13 & 0.016 & $1.4 \times 10^{-4}$ \\
\hline
...
\textbf{关键观察}：
\begin{itemize}
    \item $s = 1.5$，$L = 10$：$\eta_{\max} \approx 10^{-4}$，几乎无法训练！
