...
& SP（PyTorch默认） & $\mu$P \\
\midrule
初始化 & Kaiming: $\sigma_W^2=1/m$ & 不变（或$\sigma_W^2=1$配合前向缩放$1/\sqrt{m}$） \\
输出层缩放 & $1/\sqrt{m}$ & $\mathbf{1/m}$ \\
隐藏层学习率 & $\eta$（统一） & $\mathbf{\eta/m}$（Adam下；SGD下不缩放） \\
输出层学习率 & $\eta$ & $\eta$ \\
最优$\eta^*$ & 随$m$漂移 & \textbf{不依赖$m$} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{NTK vs $\mu$P：工程写法的主要差别}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l|cc}
\toprule
& NTK参数化 & $\mu$P（Adam下的工程写法） \\
\midrule
...
\textbf{注：}隐藏层$\eta/m$是Adam下的结果。SGD下隐藏层学习率不需要随$m$缩放（$\gamma=0$）。详见后面的推导。

``Maximal Update''的含义\textbf{不是}``更新越大越好''，而是：

\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black]
在所有使训练动力学稳定、且宽度极限非退化的参数化中，$\mu$P取\textbf{临界的最大更新尺度}。
\end{tcolorbox}

更具体地说，$\mu$P追求的是：
\begin{itemize}[leftmargin=*]
\item 前向传播稳定（激活与logits不爆炸）；
\item 反向传播稳定（梯度量级不爆炸）；
\item 训练动力学非退化（宽度变大后，参数与特征的演化不收缩到0）；
\item 超参数可迁移（不同宽度下最优学习率保持同一量级）。
\end{itemize}
因此，``maximal''意味着把稳定性约束推到\textbf{边界}，而不是取一个更保守的内点。

%=============================================================================
\section{$\mu$P的推导：从前向传播到特征变化}
%=============================================================================

\begin{teachingtip}
这一节的推导路线参考了苏剑林的清晰表述：前向稳定$\to$反向稳定$\to$损失增量$\to$特征变化，逐步加约束。每一步都有明确的物理动机，比abc框架更容易让学生跟上。
\end{teachingtip}

\subsection{模型假设}

我们考虑一个接近实践的模型簇。输入$X\in\R^{b\times d_{\text{in}}}$，输出$Z\in\R^{b\times d_{\text{out}}}$，其中$d_{\text{in}}, d_{\text{out}}$由数据决定（不可变），唯一可调的是隐藏层宽度$d$：
\begin{equation}
Y_{\text{in}} = XW_{\text{in}}, \qquad Y_{\text{out}} = \text{NN}(Y_{\text{in}};\,\Omega), \qquad Z = Y_{\text{out}}W_{\text{out}},
\end{equation}
其中$W_{\text{in}}\in\R^{d_{\text{in}}\times d}$，$W_{\text{out}}\in\R^{d\times d_{\text{out}}}$，$\text{NN}$是任意$\R^d\to\R^d$的神经网络（如$K$层MLP：$Y_{k+1}=\sigma(Y_k W_{k+1})$，$W_k\in\R^{d\times d}$）。

\textbf{关键简化：}$d_{\text{in}}, d_{\text{out}}$是常数（与$d$无关），所以分析中只需追踪与$d$相关的量级。$\text{NN}$内部的参数$W_k$都是$d\times d$方阵。

我们用\textbf{RMS}（Root Mean Square）作为矩阵``大小''的指标：$\text{RMS}(W) = \sqrt{\frac{1}{n_{\text{rows}}\cdot n_{\text{cols}}}\sum_{i,j}W_{i,j}^2}$。当$\text{RMS}(W) = \Theta(1)$时，我们说$W$是``稳定''的。

\subsection{约束1：前向传播稳定}

\begin{handcalc}{6a：前向传播的稳定性（约5分钟）}

考虑线性层$Y=XW$，$X\in\R^{b\times d_{\text{in}}}$，$W\in\R^{d_{\text{in}}\times d_{\text{out}}}$。

要让$\text{RMS}(Y)\approx\text{RMS}(X)$，需要什么样的$W$？

\tcblower

$Y_{ij} = \sum_{k=1}^{d_{\text{in}}} X_{ik}W_{kj}$。如果$X$的元素$\sim\Theta(1)$，$W$的元素独立且方差$\sigma_W^2$，则
\[
\text{Var}[Y_{ij}] = d_{\text{in}}\cdot\text{RMS}(X)^2\cdot\sigma_W^2.
\]
要让$\text{RMS}(Y)=\Theta(1)$：$\sigma_W^2 = \Theta(1/d_{\text{in}})$。

这就是\textbf{fan\_in初始化}（LeCun init），即``方差$\propto 1/d_{\text{in}}$''。

\textbf{对我们的模型簇：}
\begin{itemize}[leftmargin=*]
\item $W_{\text{in}}\in\R^{d_{\text{in}}\times d}$：$d_{\text{in}}$是常数，fan\_in方差$=\Theta(1/d_{\text{in}})=\Theta(1)$。
\item $W_k\in\R^{d\times d}$：fan\_in方差$=\Theta(1/d)$。
\item $W_{\text{out}}\in\R^{d\times d_{\text{out}}}$：fan\_in方差$=\Theta(1/d)$——但这里有选择空间，先记着。
\end{itemize}

\textbf{关于激活函数：}非线性层$Y=\sigma(XW)$只引入一个与$d$无关的常数因子（如ReLU的$\sqrt{2}$），所以\textbf{激活函数不影响$d$方向的缩放规律}，分析中可以忽略。
\end{handcalc}

\subsection{约束2：损失增量$\Delta L$不随$d$爆炸}

\begin{handcalc}{6b：损失增量的量级（约10分钟）}

参数$W\to W+\Delta W$时，损失的一阶变化：
\[
\Delta L \approx \left\langle\frac{\partial L}{\partial W},\;\Delta W\right\rangle_F = \sum_{i,j}\frac{\partial L}{\partial W_{ij}}\Delta W_{ij}.
\]
对梯度下降$\Delta W = -\eta\frac{\partial L}{\partial W}$：
\[
...
\textbf{关键：}$\|G\|_F^2 = (\text{元素个数})\times\text{RMS}(G)^2$。

对每个参数块分别分析。先看\textbf{隐藏层}$W_k\in\R^{d\times d}$：

$\frac{\partial L}{\partial W_k}$是前向信号和反向信号的乘积。如果前向和反向都稳定（$\text{RMS}=\Theta(1)$），则$\text{RMS}\!\left(\frac{\partial L}{\partial W_k}\right)$取决于$W_{\text{out}}$的初始化——暂时记为$\sigma_{\text{out}}^2$。

\textbf{（这是推导的关键分支点。$\sigma_{\text{out}}^2$的选择将最终区分NTK和$\mu$P。）}

\textbf{情况A：$\sigma_{\text{out}}^2=\Theta(1/d)$（NTK/标准选择）。}

反向传播经过$W_{\text{out}}$时，$\text{RMS}(\partial L/\partial Y_{\text{out}})=\Theta(1)$（因为$\sigma_{\text{out}}^2\cdot d = \Theta(1)$刚好抵消）。

$\Rightarrow$ $\text{RMS}(\partial L/\partial W_k) = \Theta(1)$。

$\Rightarrow$ $\|\partial L/\partial W_k\|_F^2 = d^2\times\Theta(1) = \Theta(d^2)$。

$\Rightarrow$ $\Delta L|_{W_k} = -\eta\cdot\Theta(d^2)$。要让$\Delta L=\Theta(1)$，需要$\eta\propto 1/d^2$。

\textbf{情况B：$\sigma_{\text{out}}^2=\Theta(1/d^2)$（$\mu$P选择）。}

$\text{RMS}(\partial L/\partial Y_{\text{out}}) = \Theta(1/d)$（$W_{\text{out}}$更小，反向信号衰减了$1/d$）。

$\Rightarrow$ $\text{RMS}(\partial L/\partial W_k) = \Theta(1/d)$。

$\Rightarrow$ $\|\partial L/\partial W_k\|_F^2 = d^2\times\Theta(1/d^2) = \Theta(1)$。

$\Rightarrow$ $\Delta L|_{W_k} = -\eta\cdot\Theta(1)$。$\Delta L=\Theta(1)$自动成立，\textbf{学习率$\eta$不需要随$d$缩放！}

\medskip
现在看\textbf{输出层}$W_{\text{out}}\in\R^{d\times d_{\text{out}}}$：

情况B下：$\text{RMS}(\partial L/\partial W_{\text{out}})=\Theta(1)$，元素个数$d\times d_{\text{out}}=\Theta(d)$，$\|\partial L/\partial W_{\text{out}}\|_F^2=\Theta(d)$。

$\Rightarrow$ $\Delta L|_{W_{\text{out}}} = -\eta_{\text{out}}\cdot\Theta(d)$。要让$\Delta L=\Theta(1)$：$\eta_{\text{out}}\propto 1/d$。

\medskip
\textbf{（提问学生：情况A和B的区别本质上在哪？——$W_{\text{out}}$的初始化方差！$1/d$ vs $1/d^2$。这个选择决定了反向传播信号经过输出层时衰减多少，从而决定了隐藏层梯度的量级，进而决定了$\eta$是否需要随$d$缩放。）}
\end{handcalc}

\subsection{约束3：特征变化$\Delta Y_k$稳定（$\mu$P的关键约束）}

到目前为止，$\sigma_{\text{out}}^2=\Theta(1/d)$（情况A）和$\sigma_{\text{out}}^2=\Theta(1/d^2)$（情况B）都能让$\Delta L=\Theta(1)$——只是$\eta$的缩放不同。那为什么$\mu$P选择情况B？

\begin{handcalc}{6c：特征变化约束（约8分钟）}

\textbf{动机。}损失$L$是模型的宏观/外部指标，单看$\Delta L$不够——我们还需要模型\textbf{内部}的特征（每层的激活值）变化也不随$d$退化。

考虑隐藏层$Y_k = Y_{k-1}W_k$，参数$W_k\to W_k+\Delta W_k$带来的特征变化：
\[
\Delta Y_k = Y_{k-1}\Delta W_k.
\]

$Y_{k-1}\in\R^{b\times d}$，$\Delta W_k\in\R^{d\times d}$。$\Delta W_k$是梯度下降的更新量（\textbf{不是随机的}，与$Y_{k-1}$有相关性），所以$Y_{k-1}\Delta W_k$的每个元素是$d$维向量对的内积，大小为$\Theta(d\cdot\text{RMS}(\Delta W_k))$。

\tcblower

要让$\text{RMS}(\Delta Y_k)=\Theta(1)$：
\[
\boxed{\text{RMS}(\Delta W_k) = \Theta(1/d).}
\]

\textbf{代入检验。}$\Delta W_k = -\eta\frac{\partial L}{\partial W_k}$，所以$\text{RMS}(\Delta W_k)=\eta\cdot\text{RMS}(\partial L/\partial W_k)$。

\textbf{情况A}（$\sigma_{\text{out}}^2=1/d$）：$\text{RMS}(\partial L/\partial W_k)=\Theta(1)$，要让$\eta\cdot\Theta(1)=\Theta(1/d)$，需要$\eta=\Theta(1/d)$。$\checkmark$但这时$\Delta L = -\eta\Theta(d^2) = \Theta(-d)$——\textbf{每步损失变化正比于$d$，不稳定！}（除非$\eta$再缩小到$1/d^2$，但那样$\text{RMS}(\Delta W_k)=\Theta(1/d^2)$，特征变化退化。）

\textbf{情况B}（$\sigma_{\text{out}}^2=1/d^2$）：$\text{RMS}(\partial L/\partial W_k)=\Theta(1/d)$，学习率$\eta=\Theta(1)$（不缩放！），$\text{RMS}(\Delta W_k)=\Theta(1)\times\Theta(1/d)=\Theta(1/d)$。$\checkmark$

$\Delta L = -\eta\Theta(1) = \Theta(-1)$。$\checkmark$

\textbf{所有约束同时满足！}

\medskip
\begin{tcolorbox}[colback=red!5,colframe=red!60!black,title=\textbf{$\mu$P的推导结论}]
\textbf{情况B是唯一的一致选择：}$W_{\text{out}}$的初始化方差$\propto 1/d^2$（而非$1/d$），使得：
\begin{itemize}[leftmargin=*]
\item 前向传播稳定（$\text{RMS}$逐层不变）；
\item 损失增量$\Delta L=\Theta(1)$（不随$d$爆炸）；
\item 隐藏层特征变化$\Delta Y_k$也是$\Theta(1)$（feature learning不退化）；
\item 隐藏层学习率$\eta$不随$d$缩放。
\end{itemize}

情况A（标准参数化/NTK）中$W_{\text{out}}$的方差$\propto 1/d$太大——反向信号太强，导致$\eta$必须随$d$缩小来控制$\Delta L$，但这同时压制了特征变化。\textbf{这就是SP下feature learning退化、且$\eta^*$随$d$漂移的根本原因。}
\end{tcolorbox}
\end{handcalc}

\subsection{SGD版本的$\mu$P规则汇总}

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{l|ccc}
\toprule
& $W_{\text{in}}\in\R^{d_{\text{in}}\times d}$ & $W_k\in\R^{d\times d}$（隐藏层） & $W_{\text{out}}\in\R^{d\times d_{\text{out}}}$ \\
\midrule
初始化方差 & $\Theta(1)$（$d_{\text{in}}$是常数） & $\Theta(1/d)$ & $\Theta(1/d^2)$ \\
$\text{RMS}(\partial L/\partial W)$ & $\Theta(1/d)$ & $\Theta(1/d)$ & $\Theta(1)$ \\
$\|G\|_F^2$（元素个数$\times$RMS$^2$） & $\Theta(d_{\text{in}}\cdot d\cdot 1/d^2)=\Theta(1/d)$ & $\Theta(d^2\cdot 1/d^2)=\Theta(1)$ & $\Theta(d\cdot 1)=\Theta(d)$ \\
SGD学习率$\eta$ & $\eta_{\text{in}}\propto d$（放大！） & $\eta$（不缩放） & $\eta_{\text{out}}\propto 1/d$ \\
\bottomrule
\end{tabular}
\end{center}

\begin{remark}{$W_{\text{in}}$的学习率要放大？}{win-lr}
是的。$W_{\text{in}}$的梯度$\|\partial L/\partial W_{\text{in}}\|_F^2=\Theta(1/d)$太小（因为$d_{\text{in}}$是常数而$d$在增大），SGD下需要$\eta_{\text{in}}\propto d$来补偿。这看起来反直觉，但在实际$\mu$P实现中，这通常通过把$1/d$因子吸收进$W_{\text{out}}$的参数化来避免显式的大学习率（见下面的``工程写法''）。
\end{remark}

\subsection{Adam版本（实践中更常用）}

Adam的缩放分析可以用\textbf{SignSGD近似}：$\Delta W\approx -\eta\,\text{sign}(\partial L/\partial W)$，$\Delta L\approx -\eta|\partial L/\partial W|_1$（绝对值之和）。

由于$|\cdot|_1 = (\text{元素个数})\times\text{Mean}|G_{ij}|$，量级分析变为：

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{l|ccc}
\toprule
& $W_{\text{in}}$ & $W_k$（隐藏层） & $W_{\text{out}}$ \\
\midrule
$|G|_1$ & $\Theta(d_{\text{in}}\cdot d\cdot 1/d)=\Theta(1)$ & $\Theta(d^2\cdot 1/d)=\Theta(d)$ & $\Theta(d\cdot 1)=\Theta(d)$ \\
Adam学习率$\eta$ & $\eta_{\text{in}}$不缩放 & $\eta/d$ & $\eta_{\text{out}}\propto 1/d$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Adam vs SGD的区别：}Adam的sign操作把每个元素归一化到$\pm 1$，所以$\Delta W$的RMS是$\Theta(1)$（不是$\Theta(\text{RMS}(G))$），特征变化条件$\text{RMS}(\Delta W_k)=\Theta(1/d)$需要学习率$\eta_k\propto 1/d$来实现。

\begin{remark}{学习率缩放指数$\gamma$不是普适常数！}{gamma-not-universal}

你可能看到``$\mu$P下隐藏层学习率$=\eta/m$''就以为$\gamma=1$是定论。\textbf{不是。}

更一般地，每个参数块$\ell$的学习率可以写成
\[
\eta_\ell = \frac{\eta}{m^\gamma},
\]
其中指数$\gamma$由三个因素\textbf{共同决定}：
\begin{enumerate}[leftmargin=*]
\item 优化器对应的loss increment近似形式（SGD用$\|G\|_F^2$，Adam/SignSGD用$|G|_1$，Muon用$\|G\|_*$）；
\item 该参数块的形状（$d_{\text{in}}\times d$ vs $d\times d$ vs $d\times d_{\text{out}}$）；
\item 该参数块梯度元素的量级（取决于$W_{\text{out}}$初始化的选择）。
\end{enumerate}

\textbf{具体例子：对中间方阵隐藏层$W_k\in\R^{m\times m}$，$\text{RMS}(\partial L/\partial W_k)=\Theta(1/m)$：}

\medskip
\textbf{Adam/SignSGD：}
\[
\left|\frac{\partial L}{\partial W_k}\right|_1 = m^2\cdot\text{Mean}|G_{ij}| = m^2\cdot\Theta(1/m) = \Theta(m).
\]
要让$\Delta L = -\eta_k|G|_1 = \Theta(1)$：$\eta_k=\Theta(1/m)$，即$\boxed{\gamma=1}$。

\medskip
\textbf{SGD：}
\[
\left\|\frac{\partial L}{\partial W_k}\right\|_F^2 = m^2\cdot\Theta(1/m^2) = \Theta(1).
\]
要让$\Delta L = -\eta_k\|G\|_F^2 = \Theta(1)$：$\eta_k=\Theta(1)$，即$\boxed{\gamma=0}$。

\medskip
\[
\boxed{\gamma=1\text{ 不是普遍真理，而是特定层 + 特定优化器 + 特定稳定性要求下推出的结果。}}
\]

完整的$\gamma$取值汇总（对$W_k\in\R^{m\times m}$隐藏层）：

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l|ccc}
\toprule
优化器 & loss increment近似 & $\Delta L$量级 & 隐藏层$\gamma$ \\
\midrule
SGD & $-\eta\|G\|_F^2$ & $\Theta(\eta\cdot 1)$ & 0 \\
Adam/SignSGD & $-\eta|G|_1$ & $\Theta(\eta\cdot m)$ & 1 \\
Muon/MSignSGD & $-\eta\|G\|_*\approx -\eta\|G\|_F$ & $\Theta(\eta\cdot 1)$ & 0 \\
\bottomrule
\end{tabular}
\end{center}

\textbf{（注：实践中绝大多数人用Adam，所以``$\mu$P隐藏层$\eta/m$''几乎成了默认说法。但作为理论课，必须讲清楚这背后的依赖关系。）}
\end{remark}

\begin{classexercise}{5a：检验特征变化条件（5分钟）}
\begin{enumerate}[label=(\alph*)]
\item Adam下$\Delta W_k = -\eta_k\cdot\text{sign}(G_k)$，$\text{RMS}(\Delta W_k) = \eta_k$。要让$\text{RMS}(\Delta W_k)=\Theta(1/d)$，需要$\eta_k=?$
\item SGD下$\Delta W_k = -\eta_k G_k$，$\text{RMS}(\Delta W_k)=\eta_k\cdot\text{RMS}(G_k)=\eta_k\cdot\Theta(1/d)$。要让$=\Theta(1/d)$，需要$\eta_k=?$
\item 为什么Adam的$\mu$P需要缩放隐藏层学习率$\eta_k\propto 1/d$，而SGD不需要？
\end{enumerate}

\textbf{答案：}(a) $\eta_k=\Theta(1/d)$，即$\gamma=1$。(b) $\eta_k=\Theta(1)$，即$\gamma=0$。(c) Adam的sign操作``抹平''了梯度的原始量级（每个分量变成$\pm 1$），需要学习率来补偿；SGD保留了梯度的量级，不需要额外补偿。这就是上面remark中$\gamma$取值不同的根本原因。
\end{classexercise}

\subsection{工程$\mu$P写法}

实践中常见的$\mu$P写法把部分缩放吸收进输出层参数化：
...
& 上面的推导版 & 工程$\mu$P（等效） \\
\midrule
$W_{\text{out}}$初始化方差 & $1/d^2$ & $1/d$（标准fan\_in） \\
输出缩放 & 无额外缩放 & 乘以$1/\sqrt{d}$（即$Z=\frac{1}{\sqrt{d}}Y_{\text{out}}W_{\text{out}}$） \\
\bottomrule
\end{tabular}
\end{center}

这两种写法产生相同的初始输出量级和训练动力学。工程写法的优点是$W_{\text{out}}$用标准初始化，更方便实现。

对两层网络$f=\frac{1}{m}\sum_j a_j\sigma(w_j^\top x)$，这里$1/m$对应$1/d^2$的$W_{\text{out}}$方差（因为$a_j\sim\cN(0,1)$，$1/m$的前缩放等效于方差$1/m^2$的输出层）。

%=============================================================================
\section{NTK vs $\mu$P：训练时间尺度的本质区别}
%=============================================================================

\begin{handcalc}{7：两种参数化的量级对比（课堂核心，约20分钟）}

\textbf{设置。} 两层ReLU网络，$m$个隐藏神经元，标量输入$d=1$。

\textbf{NTK参数化：}$f_{\text{NTK}}(x) = \frac{1}{\sqrt{m}}\sum_j a_j\text{ReLU}(w_j x)$，所有参数学习率$\eta$。

\textbf{工程$\mu$P：}$f_{\mu P}(x) = \frac{1}{m}\sum_j a_j\text{ReLU}(w_j x)$，$a_j$学习率$\eta$，$w_j$学习率$\eta/m$。

\textbf{本节目标：}不是比较``谁的单步更大''，而是比较\textbf{训练时间尺度}上的本质差别。
...
\begin{tabular}{l|c|c}
\toprule
& \textbf{NTK} & \textbf{$\mu$P} \\
\midrule
$\partial f/\partial a_j$ & $h_j/\sqrt{m}=O(m^{-1/2})$ & $h_j/m=O(m^{-1})$ \\
$\partial f/\partial w_j$ & $a_j\mathbf{1}\cdot x/\sqrt{m}=O(m^{-1/2})$ & $a_j\mathbf{1}\cdot x/m=O(m^{-1})$ \\
\hline
$\Delta a_j$（单步） & $O(\eta m^{-1/2})$ & $O(\eta m^{-1})$ \\
$\Delta w_j$（单步） & $O(\eta m^{-1/2})$ & $O(\eta m^{-2})$（含$\eta_w=\eta/m$） \\
\bottomrule
\end{tabular}
\end{center}

\textbf{观察：}$\mu$P的单步更新比NTK更小。如果只看单步，会误以为$\mu$P更``lazy''。

\textbf{但这是误区。}判断是否进入lazy regime，不能只看单步大小，而要看：
\[
\boxed{\text{在训练时间尺度上，参数与特征的累计位移是否仍趋于0。}}
\]

NTK的特点是：虽然单步$\Delta f=O(\eta)$很大，但参数始终停留在初始化附近（$|\Delta w_j|=O(1/\sqrt{m})\to 0$），训练可由冻结核近似描述。

$\mu$P的设计目标是：单步保持稳定，但在合适的训练时间尺度上，参数与特征的累计变化不退化到0。

\begin{center}
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{l|c|c}
\toprule
\textbf{性质} & \textbf{NTK} & \textbf{$\mu$P} \\
\midrule
单个$a_j$单步变化 & $O(\eta m^{-1/2})$ & $O(\eta m^{-1})$ \\
单个$w_j$单步变化 & $O(\eta m^{-1/2})$ & $O(\eta m^{-2})$ \\
\hline
宽度极限下参数是否停在初始化邻域 & 是 & 否（训练时间尺度上） \\
核是否冻结 & 是 & 否（一般会漂移） \\
训练本质 & lazy / kernel regression & non-lazy / feature learning \\
\bottomrule
\end{tabular}
\end{center}

\textbf{结论：}$\mu$P不是靠``更大的单步''摆脱NTK，而是靠\textbf{稳定的单步 + 非退化的长期积累}摆脱lazy training。
\end{handcalc}

\begin{handcalc}{8：单步量级的具体数字（$m=100$，约8分钟）}

取$m=100, \eta=0.1$，某个神经元$a_j=1, w_j=0.5, x=1$，$h_j=\text{ReLU}(0.5)=0.5$。设残差$(f-y)=1$。只比较\textbf{单步}量级。

\tcblower

\textbf{NTK参数化：}
\[
\frac{\partial f}{\partial a_j} = \frac{0.5}{\sqrt{100}}=0.05, \quad \Delta a_j \sim -0.1\times 0.05 = -5\times 10^{-3}.
\]
\[
\frac{\partial f}{\partial w_j} = \frac{1}{\sqrt{100}}=0.1, \quad \Delta w_j \sim -0.1\times 0.1 = -10^{-2}.
\]

\textbf{工程$\mu$P：}
\[
\frac{\partial f}{\partial a_j} = \frac{0.5}{100}=0.005, \quad \Delta a_j \sim -0.1\times 0.005 = -5\times 10^{-4}.
\]
\[
\frac{\partial f}{\partial w_j} = \frac{1}{100}=0.01, \quad \eta_w=\frac{0.1}{100}=0.001, \quad \Delta w_j \sim -0.001\times 0.01 = -10^{-5}.
\]

\textbf{观察：}
\begin{itemize}[leftmargin=*]
\item $\mu$P下单个参数的单步变化比NTK小$\sim$10--1000倍。
\item 但$\mu$P关注的不是单步大小，而是\textbf{宽度放大后训练轨迹是否仍然非退化}。
\item 单步数值例子只能说明局部步长，不能直接判定是否发生feature learning。
\end{itemize}
\end{handcalc}

\begin{classexercise}{6：为什么NTK的``大单步''反而没有feature learning？（8分钟讨论）}
NTK下$|\Delta w_j|=O(1/\sqrt{m})$（如$m=100$时$|\Delta w_j|\approx 0.01$），比$\mu$P的$|\Delta w_j|\approx 10^{-5}$大1000倍。为什么反而是NTK没有feature learning？

\textbf{提示：}想想$m$个$\Delta w_j$的\textbf{方向}。NTK下不同神经元的梯度方向近似独立（因为初始化是随机的），所以经验分布的$W_2$变化是$O(1/\sqrt{m})\to 0$（CLT：$m$个随机方向的平均效应$\propto 1/\sqrt{m}$）。$\mu$P下$\Delta w_j$虽然更小，但$m$个神经元在\textbf{同一个梯度场}下协调运动——就像一群蚂蚁各走一小步但朝同一方向，队形可以质变。

\textbf{结论：}单个参数位移的大小$\neq$特征学习的程度。关键是位移的\textbf{协调性}。
\end{classexercise}
...
$\mu$P的核心不是让所有原始张量分量都逐项保持$O(1)$，而是让\textbf{训练动力学在宽度变化时保持同类尺度}：不同宽度下，前向信号、反向信号以及各参数块对训练轨迹的有效贡献都通过适当的参数化和逐层学习率得到补偿。因此，最优学习率等超参数不再随宽度系统性漂移，从而支持zero-shot transfer。

相比之下，SP/NTK下不同参数块的有效训练时间尺度会随宽度变化，因而最优$\eta^*$往往发生系统漂移。
\end{keyinsight}
...
隐藏层$\eta$ & $\eta$ & $\eta$ & 依定义而变 & 逐层缩放 \\
参数变化 & 依赖$m$ & 逐层趋小 & 非退化 & 非退化（训练时间尺度上） \\
特征变化 & 依赖$m$ & $\to 0$ & $O(1)$ & 非退化 \\
$m\to\infty$极限 & 不一定存在 & 存在（线性化/核冻结） & 存在（mean-field极限） & 存在（非lazy极限） \\
...
\textbf{2. $\mu$P}通过联合设计输出层参数化（$W_{\text{out}}$方差$\propto 1/d^2$而非$1/d$）和逐层学习率，使得前向/反向信号稳定、损失增量$\Delta L=\Theta(1)$、且特征变化$\Delta Y_k$不退化——从而在infinite-width极限下保留feature learning并支持超参数迁移。
